...
export const metadata = { title: "Contests â€” Admin" };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          ...
          <h1 style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: "1.75rem", color: "var(--color-text)", letterSpacing: "-0.03em" }}>Contests</h1>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1px", background: "rgba(139,92,246,0.12)", borderRadius: "14px", overflow: "hidden", border: "1px solid rgba(139,92,246,0.12)" }}>
        {[
          ...
          <div key={label} style={{ background: "var(--color-bg-surface)", padding: "20px 24px" }}>
            <div style={{ fontFamily: "var(--font-dm-mono)", fontSize: "1.75rem", fontWeight: 500, color: "var(--color-text)" }}>{value}</div>
      ...
      <div style={{ background: "var(--color-bg-surface)", border: "1px solid rgba(139,92,246,0.12)", borderRadius: "14px", overflow: "hidden" }}>
        ...
        {!contests || contests.length === 0 ? (
          <div style={{ padding: "48px", textAlign: "center" }}>
            <p style={{ color: "var(--color-text-muted)", marginBottom: "12px" }}>No contests yet</p>
            <Link href="/admin/contests/new" style={{ color: "var(--color-purple)", fontSize: "0.875rem", textDecoration: "none" }}>Create your first contest â†’</Link>
          </div>
        ) : contests.map((c, i) => {
          ...
          return (
            <div key={c.id} style={{
              ...
              <div style={{ fontFamily: "var(--font-dm-mono)", fontSize: "0.8125rem", color: "var(--color-text-muted)" }}>{artworkCount}</div>
              <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                <Link href={`/admin/contests/${c.id}`} style={{ fontSize: "0.8125rem", color: "var(--color-purple)", textDecoration: "none" }}>
                  Manage
                </Link>
