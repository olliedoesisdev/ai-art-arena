// app/contests/ai-art/[id]/submit/page.tsx [SERVER]
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = contest?.theme
    ...
    : `Submit to AI Art Contest #${contest?.contest_number} — AI Art Arena`;

  return {
    title,
    alternates: { canonical: `${SITE_URL}/contests/ai-art/${id}/submit` },
  };
}

export default async function AiArtSubmitPage({ params }: Props) {
  const { id } = await params;
  const session = await auth();

  if (!session?.user) {
    redirect(`/api/auth/signin?callbackUrl=/contests/ai-art/${id}/submit`);
  }
    ...
    return (
      <div style={{ paddingTop: "80px", paddingBottom: "80px", textAlign: "center" }}>
        <div className="shell">
          ...
          <Link href={`/contests/ai-art/${id}`} style={{ color: "var(--color-purple-light)", textDecoration: "none" }}>Go vote &rarr;</Link>
        </div>
      </div>
    );
  }
